...
package ch.maybites.tools.mathematik;
...
public final class Intersection
    ...
    public static boolean intersectRayPlane(final Ray3f theRay,
                                            ...
                                            final boolean quad) {
        Vector3f diff = ch.maybites.tools.mathematik.Util.sub(theRay.origin, thePlane.origin); // mathematik.Util.sub(theRay.origin, v0);
...
//    }
    ...
    public static boolean intersectRayTriangle(final Vector3f orig,
        ...
        edge1 = ch.maybites.tools.mathematik.Util.sub(vert1, vert0);
        edge2 = ch.maybites.tools.mathematik.Util.sub(vert2, vert0);

        /* begin calculating determinant - also used to calculate U parameter */
        pvec = ch.maybites.tools.mathematik.Util.cross(dir, edge2);

        /* if determinant is near zero, ray lies in plane of triangle */
        det = edge1.dot(pvec);

        /* calculate distance from vert0 to ray origin */
        tvec = ch.maybites.tools.mathematik.Util.sub(orig, vert0);
        inv_det = 1.0f / det;

        qvec = ch.maybites.tools.mathematik.Util.cross(tvec, edge1);
...
//        myP2.add(p1);
    ...
    public static boolean lineLineIntersect(Vector3f p1, Vector3f p2, Vector3f p3, Vector3f p4,
        ...
        final Vector3f p13 = ch.maybites.tools.mathematik.Util.sub(p1, p3);
        final Vector3f p43 = ch.maybites.tools.mathematik.Util.sub(p4, p3);
        if (Math.abs(p43.x) < EPSILON && Math.abs(p43.y) < EPSILON && Math.abs(p43.z) < EPSILON) {
            return false;
        }

        final Vector3f p21 = ch.maybites.tools.mathematik.Util.sub(p2, p1);
